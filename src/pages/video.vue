<template>
  <div class="back" >
    ...
    <select v-model="value" @change="changename"  style="background-color:rgb(0,0,0,0);
    font-size:22px;width:150px;height:50px;margin-left:400px;color:#fff;border:0px;text-indent: 10px;border: 2px solid #00b8de;">
      <option style="background-color:#161273" v-for="(index,item) in caplist" :value ="index">{{index.houseName}}</option>
    </select>
    <div style="display: flex;justify-content: center;align-items: center;margin-top: 50px;">
      <div>

        <!-- <div style="color:#fff;margin-left:40px;font-size: 27px;">测试栏</div> -->
      <dv-border-box-10 :color="['#1e2285', '#00b8de']" dur=600 style="width:550px;height:380px;margin: 10px;">
        <video
        id="myPlayer"
    ...
    <div>
    <!-- <div style="color:#fff;margin-left:40px;font-size: 27px;">测试栏</div> -->
      <dv-border-box-10 :color="['#1e2285', '#00b8de']" style="width:550px;height:380px;margin: 10px;">
        <video
        id="myPlayer2"
        style="width: 520px;height: 350px;margin-top: 20px;"
        :src="http_url2"
...
export default {
  ...
  mounted() {
    ...
    doGet(`${API_HOST}/api/datav/currencyController/getFarmHouseList?page=0&limit=0`,{authToken:localStorage.authToken,farmId:localStorage.farm_id}).then(res => {
      let that = this
      if (res.data.errcode == 200) {
        // console.log(res.data)
        //       res.data.dataSource.list.forEach(function(val,arr){
        // console.log(val.houseName)
        //       that.options.push({value:val.houseId,label:val.houseName})
        //     });
    
      this.caplist = res.data.dataSource.list
      }
    })
    // doGet(`${API_HOST}/api/system/MonitorDevice/getMonitorDeviceList`,{authToken:localStorage.authToken,houseId:this.$route.query.houseId,}).then(res => {
    //   // let that = this
    //   if (res.data.errcode == 200) {
    //     console.log(res.data)
    ...
    //   }
    // })
    doGet(`${API_HOST}/api/system/MonitorDevice/getMonitorDeviceList`,{authToken:localStorage.authToken,houseId:localStorage.eventid}).then(res => {
      let that = this
      if (res.data.errcode == 200) {
        ...
        if (res.data.dataSource.list.length > 6) {
            ...
            // console.log(max)
            if (res.data.dataSource.list.length<max) {
        ...
        // this.value =res.data.dataSource.list[0].houseName
  ...
  methods: {
    ...
    changename(event){
      ...
      localStorage['eventid'] = this.value.houseId
      localStorage['ooname'] = this.value.houseName
      localStorage.setItem('select', JSON.stringify(this.value))
      this.$router.push('/emptyPage')
      doGet(`${API_HOST}/api/system/MonitorDevice/getMonitorDeviceList`,{authToken:localStorage.authToken,houseId:this.value.houseId}).then(res => {
      let that = this
      if (res.data.errcode == 200) {
        console.log(res.data)
        that.http_url = res.data.dataSource.list[0].videoAddress
