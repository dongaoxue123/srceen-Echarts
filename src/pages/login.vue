      ...
      <!-- <div style="background: #141178;box-shadow: 0 2px 4px rgba(0, 0, 0, .12), 0 0 6px rgba(0, 0, 0, .04);height:450px;width: 550px;margin: auto;"> -->
        ...
        <div>
          <dv-border-box-7 :color="['#151849', '#3f96a5']" style="height:310px;width: 400px;margin: auto;padding-top: 20px;margin-top: 200px;">
            ...
            <div>
              <input class="inputs" style="width: 70%;height: 35px;line-height: 35px;" placeholder="账号" v-model="value" @input="changenamevalue" ></input>
            </div>
              <div style="margin-top: 50px;">
              <input class="inputs" style="width: 70%;height: 35px;line-height: 35px;" placeholder="密码" type="password" v-model="passwordValue"  @input="changepassvalue" autocomplete="off"></input>
            </div>
      ...
      submitForm(){
        doPost(`${API_HOST}/api/users/login`,{loginName:this.loginName,password:this.password}).then(res => {
          if (res.data.errcode == 200) {
            console.log(res)
             sessionStorage.setItem('user', res.data.dataSource.list[0].authToken);
            ...
            localStorage['farmNamex'] = res.data.dataSource.list[0].user.farmName
            localStorage['uid'] = res.data.dataSource.list[0].user.id
            // farmName
